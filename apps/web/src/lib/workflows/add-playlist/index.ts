...
import { buildPlaylistRssUrl, extractPlaylistId } from '@/lib/platforms/youtube/urls';

import { type PlaylistFeed, persistPlaylistVideos } from './persistPlaylistVideos';
...
export async function addPlaylistForUser(args: {
  ...
  const videosProcessed = await persistPlaylistVideos(prisma, playlist.id, feed);
