 ...
 * truncation drops the tail). The summary and article are generated
 * from this same transcript, so those tabs share the omission — hence
 * one notice under all three. Each gap deep-links into the source video
 * so the reader can watch the missing part.
 */
export default function IncompleteTranscriptNotice({ gaps, platform, sourceId }: Props) {
  if (gaps.length === 0) {
    return null;
  }
