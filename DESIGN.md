...
`app/sitemap.ts` and `app/robots.ts` are static metadata routes (`force-static`), generated once per deploy at build time against the build-time `DATABASE_URL`. The sitemap lists the marketing/legal pages plus every public video reader page (`/p/videos/[videoId]`) that actually resolves — mirroring that page's 404 rule, a video qualifies only when its *latest* transcript has a READY summary or article. Entries are newest-first (`published_at DESC NULLS LAST, id ASC` — fully specified so the output is deterministic for a given DB state), capped at `PUBLIC_VIDEO_SITEMAP_CAP` (5,000; the protocol allows 50,000 per file), and `lastModified` is the newest READY generation timestamp — a stored value, never `new Date()`, so rebuilding against an unchanged DB yields a byte-identical sitemap. Because the candidate query's `some`-transcript filter over-selects relative to the latest-transcript rule, `buildVideoSitemapEntries` re-checks and applies the cap in JS (see `lib/sitemap/publicVideoSitemap.ts`). New videos appear on the next deploy; robots.txt exists mainly to point crawlers at `/sitemap.xml`.
