...
3. **Parses and validates** (`parseGeneratedTranscript`: fences, trailing commas, token-ceiling truncation salvage, `H:MM:SS` timestamps, monotonicity + duration clamps). Both parse and validation run **inside the generate step**, not in persist, so a bad response can be re-generated — Gemini intermittently fails to fetch the video server-side and answers from the prompt text alone, inventing a short, unrelated transcript (the tell: the run's prompt carries only ~150 input tokens, no VIDEO/AUDIO modality, and covers a sliver of the video). `assertUsableGeneration` (`validateGeneratedTranscript.ts`) rejects such runs — input tokens below `TRANSCRIPT_GENERATION_MIN_INPUT_TOKENS`, or coverage below `TRANSCRIPT_GENERATION_MIN_COVERAGE_RATIO` of a known duration when not truncated by the token ceiling — by throwing a **retryable** (non-`FatalError`) error, so the WDK step retry re-runs the model and usually lands a real transcription. Unparseable output stays `FatalError` (deterministic).
4. **Persists** the validated segments with `Transcript.source = GENERATED`, which renders an "AI-generated transcript" badge in the reader.
